...
Cartridge::Cartridge(const char* filename)
{
    ...
    // Narya port: filename is the ROM directory entry name. Map it through
    // rom_store so we get a flash-XIP virtual pointer for the entire image.
    static esp_partition_mmap_handle_t s_handle;
    if (s_handle) {
        rom_store_munmap(s_handle);
        s_handle = 0;
    }
    uint32_t mapped_size = 0;
    rom_data = rom_store_mmap(filename, &s_handle, &mapped_size);
    if (!rom_data) {
        is_valid = false;
        return;
    }
    rom_size = mapped_size;

    if (rom_size < sizeof(cartridge_header)) {
        is_valid = false;
        return;
    }
    memcpy(&header, rom_data, sizeof(cartridge_header));
    ...
    chr_base = prg_base + (uint32_t)number_PRG_banks * 16384u;

    // CRC32 over the cartridge body (PRG + CHR), straight off the mmap.
    if (rom_size > prg_base) {
        CRC32 = crc32(rom_data + prg_base, rom_size - prg_base, CRC32) ^ ~0U;
        LOGF("CRC32: %08lX", (unsigned int)CRC32);
    }
