...
inline void Apu2A03::generateSample()
{
    ...
    {
        buffer_index = 0;
        // Narya port: hand the freshly produced stereo block to the I2S
        // pipeline maintained outside Anemoia. AUDIO_BUFFER_SIZE is the
        // count of stereo *frames*; the buffer holds 2x that many uint16
        // samples (interleaved L/R).
        narya_anemoia_publish_audio((const int16_t*)audio_buffer,
                                    AUDIO_BUFFER_SIZE * 2);
    }
}
